...
Feature: Verify build from scratch Module
  ...
  Scenario Outline: Verify build from scratch functionality
    ...
    And User sets <actionRepeatable>,<status>,<tags>,<trackingType>,<trackingInstruction>,<imageTrackingType>,<filterByTags> and <iconType>
    ...
    Then Verify all the details <name>,<points>,<corelatedTask> and <tags> are updated accordingly

    Examples: 
      | email                        | password  | name                      | shortDescription                            | actionDescription                                                                                                                                                                                                                                                                                                                                                                | startDate    | endDate      | actionRepeatable | status   | trackingType | trackingInstruction | imageTrackingType    | filterByTags | iconType  | optionalMail                 | content | tags      | corelatedTask          | points | gallonsOfWater | kgsOfPlastic | treesPlanted |
      | abhik.khandelwal01@gmail.com | Rippl@321 | "Fitness Bingo Challenge" | "Engage in a variety of fitness activities" | "Participants will receive a bingo card with various fitness-related activities in each square, such as jogging for 30 minutes, completing a yoga session, or doing 50 push-ups. They must complete these activities to mark off the corresponding squares on their card. The goal is to achieve a bingo pattern (horizontal, vertical, or diagonal) within the given timeframe" | "05-27-2024" | "06-26-2024" | "No"             | "Active" | "Image"      | "test"              | "Live image capture" | "base_tier"  | "Default" | abhik.khandelwal01@gmail.com | test    | "#Action" | Drink a glass of water |      1 |              1 |            1 |            1 |
